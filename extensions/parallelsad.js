...
ParallelLayout.prototype.isSplit = function (node) {
  ...
  var ew = myDiagram.model.linkDataArray.find(item => item.to === rt.key)
  //var ww = myDiagram.model.nodeDataArray.find(item => item.key === ew.from)
  console.log(ew);
  console.log(ParallelLayout.prototype);
...
ParallelLayout.prototype.isMerge = function (node) {
  if (!(node instanceof go.Node)) return false;
  var cat = node.category;
  return (cat === "Merge" || cat === "End" || cat === "EndFor" || cat === "EndWhile" || cat === "EndIf" || cat === "Endlhile");
}
/**
 ...
 * @return {boolean}
 */
ParallelLayout.prototype.isWhile = function (node) {
  if (!(node instanceof go.Node)) return false;
  console.log(node.category);
  return node.category === "While";
}
...
/**
 * Return an Array holding the Split node and the Merge node for this layout.{@link #isWhile}
 * This signals an error if there is not exactly one Node that {@link #isSplit}
 * and exactly one Node that {@link #isMerge}.
 * This can be overridden; any override must set {@link #splitNode} and {@link #mergeNode}. {@link #whileNode}
 * @param {*} vertexes
 */
ParallelLayout.prototype.findSplitMerge = function (vertexes) {//สร้างอันเริ่มแรก
  var split = null;
  var merge = null;
  var While = null;
  ...
  while (it.next()) {
    ...
    if (this.isSplit(v.node)) {
      ...
      merge = v.node;
    } else if (this.isWhile(v.node)) {
      if (merge) throw new Error("Merge node already exists in " + this + " -- existing: " + While + " new: " + v.node);
    }
  }
  if (!split) throw new Error("Missing Split node in " + this);
  if (!merge) throw new Error("Missing Merge node in " + this);
  //if (!While) throw new Error("Missing While node in " + this);
  this._splitNode = split;
  this._mergeNode = merge;
  this._WhileNode = While;
}
 ...
 * @param {*} coll
 */
ParallelLayout.prototype.makeNetwork = function (coll) {//จัดเรียงโนดอื่นๆ แก้ตรงนี้โว้ย
  ...
  while (it.next()) {//เอาออกแล้วเบี้ยว
    var v = it.value;
    var g = v.node;
  ...
  this.findSplitMerge(net.vertexes);//ifพังwhileพัง
  // don't have TreeLayout lay out the Merge node; commitNodes will do it
  if (this.mergeNode) net.deleteNode(this.mergeNode);//ความยาวแปลก
  if (this.splitNode) {//ไม่รู้
  ...
  return net;//หายหายหมด
};

/**
 * @hidden @internal
 */
ParallelLayout.prototype.commitNodes = function () {//endหาย+เส้นแปลกๆ//กำหนดระยะความยาวเส้น
 ...
 * @hidden @internal
 */
ParallelLayout.prototype.commitLinks = function (node) {//ถ้าหายจะเป็นเส้นตรง
  var splitNode = this.splitNode;
  var mergeNode = this.mergeNode;
  if (splitNode === null || mergeNode === null || WhileNode === null ||this.network === null) return;
  // set default link spots based on this.angle
  var it = this.network.edges.iterator;
  console.log(rt);
  console.log(this);
  var whle = this.isWhile(splitNode);
  /*var ew =myDiagram.model.linkDataArray.find(item => item.to === rt.key);
  console.log(ew);
  var te=myDiagram.model.nodeDataArray.find(item => item.key === ew.from);
  console.log(te);*/
  while (it.next()) {//ไม่รู้//แต่แก้แล้วได้//แก้หมู่
    var e = it.value;
    var link = e.link;
    ...
    if (whle) {
      ...
      if (this.angle === 0) {
        ...
        if (this.setsChildPortSpot) link.toSpot = go.Spot.Right;
      } else if (this.angle === 90) {
        if (this.setsPortSpot) link.fromSpot = go.Spot.Left;
  ...
  if (mergeNode) {
    ...
    while (it.next()) {//โดนswitch
      ...
      link.updateRoute();
      if (link.pointsCount >= 6) {//linkของswitchพันกัน
    ...
    var it = mergeNode.findLinksOutOf();//หาลิงค์ออกและเข้า

    while (it.next()) {//โดนพวกloop
      var link = it.value;
